 ...
 function clicked() {
    ...
    if(t == 0){
       ...
       // update(quizInDB1,{"score":`${score}`});
         ...
         update(quizInDB2,{"a":N,"challenge":"true","aScore":`${score}`,"aname":`${sNum}`});
        QH.innerHTML = "Your score: "+`${score}`;
        div2.innerHTML = "";
    p1.innerHTML = "waiting for opponent to complete challenge....";
    p2.innerHTML = "";
    p3.innerHTML = "" ;
    p4.innerHTML = "click the challenge button for another challege nyana";
    
    }
    else{
         const quizInDB2 = ref(database, "Quiz/"+`${items[N-1][1].a}`+"/");
                    const quizInDB1 = ref(database, "Quiz/"+`${N}`+"/");
                    let ai = items[N-1][1].a ;
                    
                    update(quizInDB1,{"aname":"none","aScore":"0","challenge":"false"})
        QH.innerHTML = "Your score: "+`${score}`;
        div2.innerHTML = "";
        p1.innerHTML = `${items[N-1][1].aname}`+ " scored "+ `${items[N-1][1].aScore}`;
    p2.innerHTML = "";
    p3.innerHTML = "" ;
    p4.innerHTML = "Take Screenshot nyana and show " + `${items[ai-1][1].name}`;
